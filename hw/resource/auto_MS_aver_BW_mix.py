...
class BwMix(object):
    ...
    def pre_write(self):
        pre_write_file = "IOMeter_mix_precondition.bat"
    ...
    def average_bw(self,):
        output_header_list = ['filename', 'Total I/Os per Second', 'average']
        if sys.version >= '3':
    ...
    def run_test(self):
        ...
        while True:
            ...
            tmp = float(self.aver_value) - self.bw_value
            print("The set value {} minus average value {} equal : {}".format(50000, self.aver_value, tmp))
            if (tmp >= -2000) and (tmp <= 2000):
                ...
                self.local_run_iometer()
                self.average_bw()
            elif (tmp > 2000) and (tmp <= 5000):
                print("the current pwm value {},please decrease the pwm value".format(pwm))
                pwm -= 1
                self.set_pwm(pwm)
                self.local_run_iometer()
                self.average_bw()
            elif (tmp >= -5000) and (tmp < -2000):
                print("the current pwm value {},please increase the pwm value".format(pwm))
                pwm += 1
                self.set_pwm(pwm)
                self.local_run_iometer()
