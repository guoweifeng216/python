...
# 购物车结算系统
def buy_account(buy_item, if_vip):
...
def buyer_log_manager_sys(user_id, money, *item):
...
def shopping_hall():
    ...
    while True:
        ...
        if_buy = raw_input("请输入y或者n选择是否继续购物：")
        if if_buy == "n":
            if len(buy_car) > 0:
                total_money = buy_account(buy_car)
