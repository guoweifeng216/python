...
class BwWrite(object):
    ...
    def local_run_iometer(self):
        read_file = "IOMeter_seq_write.bat"
    ...
    def local_run_iometer_15m(self):
        read_file = "IOMeter_seq_write_15m.bat"
        read_cmd = "{}\{}".format(self.cmd_path, read_file)
        run_cmd = sub.call(read_cmd, shell=True)
        if run_cmd != 0:
            print("run iometer failed")
    ...
    def run_test(self):
        ...
        while True:
            ...
            print("The set value {} minus average value {} equal : {}".format(800, self.aver_value, tmp))
            if (tmp >= -30) and (tmp <= 30):
